...
/// diffs it against LastSent and owns the wire. Called from floating.zig's
...
pub fn moveFocused(ctx: *Ctx, delta: i32) void {
    ...
    if (next_i < 0 or next_i >= m.ws[h].tiled_order.items.len) return;
...
pub fn mapRequest(ctx: *Ctx, win: model_mod.WindowId, target_ws: u8, on_current: bool) void {
    ...
    model_mod.register(m, win, if (on_current) null else target_ws);
