...
fn setWindowProperties(wm: *defs.WM, window: u32, height: u16, want_transparency: bool, alpha: u16) !void {
    ...
    // CRITICAL: Set window opacity via _NET_WM_WINDOW_OPACITY (same as window borders)
    // This matches how window borders get their transparency - via compositor, not Cairo alpha
    // The compositor will apply gaussian blur to this opacity value
    if (want_transparency) {
        const opacity_atom = try utils.getAtom(wm.conn, "_NET_WM_WINDOW_OPACITY");
        const alpha_u32: u32 = @intCast(alpha); // Convert 16-bit to 32-bit first
        const opacity_u32 = [_]u32{alpha_u32 * 65537}; // Scale to 32-bit range (0xFFFF -> 0xFFFFFFFF)
...
pub fn init(wm: *defs.WM) !void {
    ...
    // IMPORTANT: Use RGB window (not ARGB) + _NET_WM_WINDOW_OPACITY for transparency
    // This matches window border behavior: opaque RGB + compositor transparency
    // ARGB windows with Cairo cause premultiplication issues
    const window = xcb.xcb_generate_id(wm.conn);
    
    // Create window with default RGB visual (no ARGB to avoid premultiplication)
    ...
    debug.info("Created RGB window for bar (transparency via _NET_WM_WINDOW_OPACITY)", .{});
    ...
    // Create DrawContext with regular RGB visual
    const dc = try drawing.DrawContext.init(wm.allocator, wm.conn, window, width, height, wm.dpi_info.dpi);
    errdefer dc.deinit();
    try loadBarFonts(dc, wm);
    
    // DON'T set alpha_override - transparency is handled via _NET_WM_WINDOW_OPACITY
    // This matches how window borders work: opaque colors + compositor transparency
    if (want_transparency) {
        debug.info("Bar transparency via _NET_WM_WINDOW_OPACITY at {d:.1}%", .{(@as(f32, @floatFromInt(alpha)) / 0xFFFF) * 100.0});
    }

    const s = try State.init(wm.allocator, wm.conn, window, width, height, dc, wm.config.bar, false); // transparency handled by compositor, not Cairo
    try draw(s, wm);
    utils.flush(wm.conn);
    state = s;
}
