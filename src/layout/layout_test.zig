...
const scroll_algo = @import("algo_scroll");
...
const Fixture = struct {
    ...
    fn init(self: *Fixture, wins: []const model.WindowId, wa: utils.Rect) void {
        ...
        for (wins) |w| model.register(&self.m, w, null);
        self.hv = .{ .m = &self.m };
    }
...
fn tuned(fx: *Fixture) engine.View {
    var v = fx.view();
    v.margins = .{ .gap = gap, .border = border };
    v.min_dim = min_dim;
    return v;
}
