...
pub fn tile(wm: *WM, state: *TilingState, windows: []const u32, screen_w: u16, screen_h: u16) void {
    ...
    for (windows, 0..) |win, idx| {
        ...
        if (idx < m_count) {
            ...
            // STACK AREA - FIX: Start at master_width instead of master_width + gap
            ...
            // FIX: Position stack at master_width (not master_width + gap)
            // The calcWindowDimension will add the gap from the left edge
            x = master_width;
            y = @intCast(@as(u32, gap) + @as(u32, row) * (@as(u32, win_height) + 2 * @as(u32, bw) + @as(u32, gap)));
