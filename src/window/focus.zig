...
/// setFocus itself. The reflow's configure_window calls are only flushed at
/// the caller's ungrabAndFlush, at which point the server may generate real
/// EnterNotify events for windows that slid under a stationary cursor; if
/// suppression were cleared synchronously, those crossings would arrive
/// un-suppressed and silently reassign keyboard focus. xcb_get_input_focus is
/// queued in the same flush as the reflow, and X guarantees any event
/// generated by a request is delivered before the reply to a later request —
/// so when drainTilingOpSettle() consumes this reply, the crossing events are
/// already drained and dispatched (and filtered, since suppression is still
/// active).
