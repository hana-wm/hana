...
pub fn onWMEvent(wm: *WM, event: @import("wm_bus").WMEvent) void {
    // Import minimize and workspaces directly — no window import to avoid the
    // cycle: focus → window → wm_bus → focus.
    // isOnCurrentWorkspace is available from workspaces without needing wm.
    const minimize   = @import("minimize");
    const workspaces = @import("workspaces");
    switch (event) {
        .window_mapped => |p| {
            if (p.on_current) setFocus(wm, p.win, .window_spawn);
        },
        .window_closed => |p| {
            if (!p.was_focused) return;
            // Pre-fire pointer query so the round-trip runs concurrently
            // with the focus-clear below (server grab is already held by caller).
            const ptr_cookie = xcb.xcb_query_pointer(wm.conn, wm.root);
            clearFocus(wm);
            // Attempt to focus the window under the pointer; fall back to
            // the best available minimized/workspace window.
            const reply = xcb.xcb_query_pointer_reply(wm.conn, ptr_cookie, null);
            if (reply == null) {
            ...
            // workspaces.isOnCurrentWorkspace takes only win (no wm), breaking
            // the import cycle that window.isOnCurrentWorkspace would introduce.
            if (workspaces.isOnCurrentWorkspace(child) and
                !minimize.isMinimized(wm, child))
            {
                setFocus(wm, child, .mouse_enter);
            } else {
                minimize.focusBestAvailable(wm);
            }
        },
        .window_entered => |win| {
            ...
            if (getFocused() == win) return;
            setFocus(wm, win, .mouse_enter);
        },
        else => {},
    }
}
